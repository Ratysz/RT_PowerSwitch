	...
	internal class ITab_RTEmergencyPowerSettings : ITab
	{
		...
		protected override void FillTab()
		{
			...
			list.Begin(rect);
			{
				...
				row.Begin(rectUnits);
				if (row.RadioButton("%", comp.usePercentages))
				{
					comp.usePercentages = true;
				}
				row.NewColumn();
				row.NewColumn();
				if (row.RadioButton("Wd", !comp.usePercentages))
				{
			...
			}
			{
				...
				if (row.RadioButton(Direction8Way.North.LabelShort(), comp.directionToMonitor == Direction8Way.North))
				{
					comp.directionToMonitor = Direction8Way.North;
				}
				if (row.RadioButton(Direction8Way.West.LabelShort(), comp.directionToMonitor == Direction8Way.West))
				{
					comp.directionToMonitor = Direction8Way.West;
				}
				if (row.RadioButton(Direction8Way.South.LabelShort(), comp.directionToMonitor == Direction8Way.South))
				{
					comp.directionToMonitor = Direction8Way.South;
				}
				if (row.RadioButton(Direction8Way.East.LabelShort(), comp.directionToMonitor == Direction8Way.East))
				{
